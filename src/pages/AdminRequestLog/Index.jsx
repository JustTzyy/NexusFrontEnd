...
const getStatusColor = (s) => {
    switch (s) {
        ...
        case "Pending Student Interest":     return "text-cyan-600";
        case "Teacher Assigned":             return "text-purple-600";
        case "Waiting for Teacher Approval": return "text-orange-600";
...
export default function AdminRequestHistory() {
    ...
    const handlePrintPDF = () => {
        generatePDF({
            title: "Admin Request History Report",
    ...
    const loadItems = useCallback(async () => {
        ...
        try {
            ...
            console.error(err);
            setError(err.message || "Failed to load admin request history");
            setItems([]);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <AppLayout title="Admin Request Log" onPrint={items.length > 0 ? handlePrintPDF : undefined}>
                                ...
                                {items.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={5} className="h-32 text-center text-gray-500">
                                            No admin request history found.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    items.map((item) => (
