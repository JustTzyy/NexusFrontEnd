...
export default function AvailableDaysArchive() {
    ...
    const [restoreModal, setRestoreModal] = useState({ open: false, day: null });
    ...
    const handlePrintPDF = () => {
        generatePDF({
            title: "Archived Available Days Report",
            data: filtered,
            columns: [
            ...
            companyName: "Archived Available Days Management System",
        });
    };

    const handlePrintSinglePDF = (day) => {
        if (!day) return;
        generatePersonalInfoPDF({
            title: "Archived Available Day Details",
            data: day,
            fields: [
    ...
    const loadArchived = async () => {
        ...
        setError("");
        try {
            const response = await availableDayService.getArchived({ pageNumber: 1, pageSize: 100 });
            ...
            const mappedData = data.map(d => ({
                ...
                isActive: d.isActive,
                deletedAt: d.deletedAt ? new Date(d.deletedAt).toLocaleDateString() : "—",
                createdAt: new Date(d.createdAt).toLocaleDateString()
            }));
            setDays(mappedData);
        } catch (err) {
            setError(err.response?.data?.message || err.message || "Failed to load archived days");
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmView = () => {
        if (viewModal.day) {
            navigate(`/available-days/${viewModal.day.id}`, { state: { from: "archive" } });
        }
        setViewModal({ open: false, day: null });
    };

    const confirmRestore = async () => {
        const d = restoreModal.day;
        if (!d) return;
        setRestoreModal({ open: false, day: null });
        try {
            await availableDayService.restore(d.id);
            setDays((prev) => prev.filter((item) => item.id !== d.id));
            setAlert({ show: true, type: "success", message: `"${d.dayName}" restored successfully!` });
        } catch (err) {
            setAlert({ show: true, type: "error", message: err.response?.data?.message || "Failed to restore day" });
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            await availableDayService.permanentDelete(d.id);
            setDays((prev) => prev.filter((item) => item.id !== d.id));
            setAlert({ show: true, type: "success", message: `"${d.dayName}" permanently deleted!` });
        } catch (err) {
            setAlert({ show: true, type: "error", message: err.response?.data?.message || "Failed to delete day" });
        }
    };

    /* ------------------------- filtering ------------------------- */
    const filtered = useMemo(() => {
        return applyFilters(days, { search: q, searchFields: ['dayName'], fromDate, toDate, dateField: 'deletedAt' });
    }, [days, q, fromDate, toDate]);
    ...
    return (
        <AppLayout title="Archived Days" onPrint={filtered.length > 0 ? handlePrintPDF : undefined}>
            {loading ? (
                <TableIndexSkeleton columns={5} rows={10} headers={["Day", "Status", "Sort Order", "Deleted At", "Actions"]} />
            ) : (
                <div className="space-y-6">
                    {/* Header */}
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Archived Days</h1>
                            <p className="mt-1 text-sm text-gray-600">View and manage archived available days</p>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent>Return to active days</TooltipContent>
                            </Tooltip>
                        </div>
                    </div>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="relative">
                                            ...
                                            <Input placeholder="Search archived days..." value={q} onChange={(e) => { setQ(e.target.value); resetPage(); }} className="pl-10 w-[240px]" />
                                        </div>
                                    </TooltipTrigger>
                    ...
                    <div className="border rounded-lg overflow-hidden bg-white">
                        <Table>
                            ...
                            <TableBody>
                                {paged.length === 0 ? (
                                    <TableRow><TableCell colSpan={5} className="h-32 text-center text-gray-500">No archived days found.</TableCell></TableRow>
                                ) : (
                                    paged.map((d) => (
                                        <Tooltip key={d.id}>
                                            <TooltipTrigger asChild>
                                                <TableRow className="cursor-pointer" onClick={() => setViewModal({ open: true, day: d })}>
                                                    ...
                                                    <TableCell className="text-center">
                                                        <div className="flex items-center justify-center gap-1">
                                                            <PermissionGate permission="RestoreAvailableDays"><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setRestoreModal({ open: true, day: d }); }} className="text-green-600 hover:text-green-700 hover:bg-green-50"><RotateCcw className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent>Restore day</TooltipContent></Tooltip></PermissionGate>
                                                            <PermissionGate permission="PermanentDeleteAvailableDays"><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setDeleteModal({ open: true, day: d }); }} className="text-red-600 hover:text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent>Permanently delete</TooltipContent></Tooltip></PermissionGate>
                                                        </div>
                                                    </TableCell>
                                                </TableRow>
                                            </TooltipTrigger>
            ...
            <Dialog open={viewModal.open} onOpenChange={(open) => !open && setViewModal({ open: false, day: null })}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>View Day Details</DialogTitle>
                        <DialogDescription>
                            You are about to view the archived day{" "}
                            <span className="font-semibold text-gray-900">"{viewModal.day?.dayName}"</span>. Do you want to continue?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="outline" onClick={() => handlePrintSinglePDF(viewModal.day)}>
                                    <Printer className="h-4 w-4 mr-2" />
                                    Print
                                </Button>
                            </TooltipTrigger>
                            <TooltipContent>Print day details as PDF</TooltipContent>
                        </Tooltip>
            ...
            <AlertDialog open={restoreModal.open} onOpenChange={(open) => !open && setRestoreModal({ open: false, day: null })}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Confirm Restore</AlertDialogTitle>
                        <AlertDialogDescription>
                            Are you sure you want to restore{" "}
                            <span className="font-semibold text-gray-900">"{restoreModal.day?.dayName}"</span>? This will move it back to active days.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <span className="font-semibold text-gray-900">"{deleteModal.day?.dayName}"</span>? This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
