...
export default function EmailTemplatesIndex() {
    ...
    const loadTemplates = async () => {
        ...
        setError('');
        try {
            const res = await emailTemplateService.getAll({ pageNumber: 1, pageSize: 100 });
    ...
    /* ------------------------------ actions ------------------------------ */
    const openDeleteModal = (template) => setDeleteModal({ open: true, template });
    const openViewModal = (template) => setViewModal({ open: true, template });
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setAlert({ show: true, type: 'success', message: `Template "${template.name}" archived successfully!` });
        } catch (err) {
            setAlert({ show: true, type: 'error', message: err.response?.data?.message || 'Failed to archive template' });
        }
    };

    const confirmView = () => {
        if (viewModal.template) {
            navigate(`/marketing/email-templates/${viewModal.template.id}`);
        }
        ...
        <AppLayout title="Email Templates">
            {loading ? (
                <TableIndexSkeleton
                    columns={5}
                    rows={10}
                    headers={['Name', 'Status', 'Category', 'Created At', 'Actions']}
                />
            ) : (
                <div className="space-y-6">
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-600">Manage reusable email templates</p>
                        </div>
                        <div className="flex items-center gap-4">
                            ...
                            <PermissionGate permission="CreateEmailTemplates">
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button onClick={() => navigate('/marketing/email-templates/create')} disabled={loading}>
                                        <Plus className="h-4 w-4 mr-2" />
                                        New Template
                                    </Button>
                                </TooltipTrigger>
                                <TooltipContent>Create a new email template</TooltipContent>
                            </Tooltip>
                            </PermissionGate>
                        </div>
                    </div>
                        ...
                        <DateRangeFilter
                            ...
                            rightElement={
                                <>
                                    ...
                                    <PermissionGate permission="ArchiveEmailTemplates">
                                    <div className="ml-auto">
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <Button variant="outline" onClick={() => navigate('/marketing/email-templates/archive')}>
                                                    <Archive className="h-4 w-4 mr-2" />
                                                    Archive
                                                </Button>
                                            </TooltipTrigger>
                                            <TooltipContent>View archived templates</TooltipContent>
                                        </Tooltip>
                                    </div>
                                    </PermissionGate>
                                </>
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="text-center">
                                                        <div className="flex items-center justify-center gap-1">
                                                            <PermissionGate permission="UpdateEmailTemplates">
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <Button
                                                                        variant="ghost"
                                                                        ...
                                                                        onClick={(e) => {
                                                                            e.stopPropagation();
                                                                            navigate(`/marketing/email-templates/${t.id}/edit`);
                                                                        }}
                                                                    >
                                                            ...
                                                            <PermissionGate permission="DeleteEmailTemplates">
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <Button
                                                                        variant="ghost"
            ...
            <AlertDialog
                ...
                onOpenChange={(open) => !open && setDeleteModal({ open: false, template: null })}
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
            ...
            <Dialog
                open={viewModal.open}
                onOpenChange={(open) => !open && setViewModal({ open: false, template: null })}
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            ...
                            <span className="font-semibold">{viewModal.template?.name}</span>?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
