...
export default function CampaignsArchive() {
    ...
    const [restoreModal, setRestoreModal] = useState({ open: false, campaign: null });
    ...
    /* ------------------------------ routes ------------------------------ */
    const goBack = () => navigate('/marketing/campaigns');
    ...
    const loadArchivedCampaigns = async () => {
        ...
        setError('');
        try {
            const response = await campaignService.getArchived({ pageNumber: 1, pageSize: 100 });
            ...
            const mappedData = data.map((c) => ({
                ...
                deletedAt: new Date(c.updatedAt).toLocaleDateString(),
            }));
            setCampaigns(mappedData);
        } catch (err) {
            setError(err.response?.data?.message || err.message || 'Failed to load archived campaigns');
        } finally {
            setLoading(false);
        }
    };
    ...
    /* ------------------------------ actions ------------------------------ */
    const openViewModal = (c) => setViewModal({ open: true, campaign: c });
    const openRestoreModal = (c) => setRestoreModal({ open: true, campaign: c });
    const openDeleteModal = (c) => setDeleteModal({ open: true, campaign: c });

    const confirmView = () => {
        if (viewModal.campaign) {
            navigate(`/marketing/campaigns/${viewModal.campaign.id}`, { state: { from: 'archive' } });
        }
        setViewModal({ open: false, campaign: null });
    };

    const confirmRestore = async () => {
        const c = restoreModal.campaign;
        if (!c) return;
        setRestoreModal({ open: false, campaign: null });

        try {
            await campaignService.restore(c.id);
            setCampaigns((prev) => prev.filter((item) => item.id !== c.id));
            setAlert({ show: true, type: 'success', message: `Campaign "${c.name}" restored successfully!` });
        } catch (err) {
            setAlert({ show: true, type: 'error', message: err.response?.data?.message || 'Failed to restore campaign' });
        }
    };

    const confirmDelete = async () => {
        const c = deleteModal.campaign;
        if (!c) return;
        setDeleteModal({ open: false, campaign: null });

        try {
            await campaignService.permanentDelete(c.id);
            setCampaigns((prev) => prev.filter((item) => item.id !== c.id));
            setAlert({ show: true, type: 'success', message: `Campaign "${c.name}" permanently deleted!` });
        } catch (err) {
            setAlert({ show: true, type: 'error', message: err.response?.data?.message || 'Failed to delete campaign' });
        }
    };
    ...
    return (
        <AppLayout title="Archived Campaigns">
            {loading ? (
                <TableIndexSkeleton
                    columns={5}
                    rows={10}
                    headers={['Name', 'Status', 'Segment', 'Deleted At', 'Actions']}
                />
            ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-600">
                                View and manage soft-deleted campaigns
                            </p>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent>Return to active campaigns</TooltipContent>
                            </Tooltip>
                        </div>
                    </div>
                                        ...
                                        <Tooltip key={c.id}>
                                            <TooltipTrigger asChild>
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="text-gray-600">{c.deletedAt}</TableCell>
                                                    <TableCell className="text-center">
                                                        <div className="flex items-center justify-center gap-1">
                                                            <PermissionGate permission="RestoreCampaigns">
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <Button
                                                                        variant="ghost"
                                                            ...
                                                            <PermissionGate permission="PermanentDeleteCampaigns">
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <Button
                                                                        variant="ghost"
            ...
            <Dialog open={viewModal.open} onOpenChange={(open) => !open && setViewModal({ open: false, campaign: null })}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>View Campaign Details</DialogTitle>
                        <DialogDescription>
                            You are about to view the archived campaign{' '}
                            <span className="font-semibold">{viewModal.campaign?.name}</span>. Do you want to continue?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
            ...
            <AlertDialog open={restoreModal.open} onOpenChange={(open) => !open && setRestoreModal({ open: false, campaign: null })}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Confirm Restore</AlertDialogTitle>
                        <AlertDialogDescription>
                            Are you sure you want to restore the campaign{' '}
                            <span className="font-semibold">{restoreModal.campaign?.name}</span>?
                            This will move it back to the active campaigns list.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog open={deleteModal.open} onOpenChange={(open) => !open && setDeleteModal({ open: false, campaign: null })}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to <span className="font-semibold text-red-600">permanently delete</span> the campaign{' '}
