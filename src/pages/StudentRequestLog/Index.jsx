...
const STATUS_LABEL = {
    "Waiting for Admin Approval": "In Review",
};
...
const getStatusColor = (s) => {
    switch (s) {
        case "Pending Teacher Interest":     return "text-indigo-600";
        case "Teacher Assigned":             return "text-purple-600";
        case "Waiting for Teacher Approval": return "text-amber-600";
        case "Waiting for Admin Approval":   return "text-blue-600";
        case "Confirmed":                    return "text-emerald-600";
        case "Cancelled by Student":         return "text-gray-500";
        case "Cancelled by Admin":           return "text-red-600";
...
export default function StudentRequestHistory() {
    ...
    const handlePrintPDF = () => {
        generatePDF({
            title: "Student Request History Report",
            data: items.map(l => ({ ...l, changedAt: formatDateTime(l.changedAt) })),
            columns: [
    ...
    const loadItems = useCallback(async () => {
        ...
        try {
            ...
            data = data.filter(h => !h.isAdminCreated);
            ...
            if (debouncedQ) {
                ...
                data = data.filter(h =>
                    ...
                    h.studentName?.toLowerCase().includes(sq)
                );
            }

            if (fromDate) data = data.filter(h => new Date(h.changedAt) >= new Date(fromDate));
            if (toDate)   data = data.filter(h => new Date(h.changedAt) <= new Date(toDate + "T23:59:59"));

            if (statusFilter !== "all") data = data.filter(h => h.toStatus === statusFilter);

            data = data.slice().sort((a, b) => new Date(b.changedAt) - new Date(a.changedAt));

            setTotalCount(data.length);
            setTotalPages(Math.max(1, Math.ceil(data.length / pageSize)));
            setItems(data.slice((page - 1) * pageSize, page * pageSize));
        } catch (err) {
            console.error(err);
            setError(err.message || "Failed to load student request history");
            setItems([]);
        } finally {
            setLoading(false);
        }
    ...
    const confirmView = () => {
        if (viewModal.item) navigate(`/schedule-configuration/log/${viewModal.item.id}`, { state: { item: viewModal.item } });
        setViewModal({ open: false, item: null });
    };

    return (
        <AppLayout title="Student Request Log" onPrint={items.length > 0 ? handlePrintPDF : undefined}>
            {loading && items.length === 0 ? (
                ...
                <div className="space-y-6">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Student Request Log</h1>
                            <p className="mt-1 text-sm text-gray-600">
                                Track all status changes for student-submitted tutoring requests
                            </p>
                        </div>
                    </div>
                        ...
                        <DateRangeFilter
                            ...
                            leftElement={
                                <Tooltip>
                                    ...
                                    <TooltipContent>Search by user, subject, building, or student</TooltipContent>
                                </Tooltip>
                            }
                            rightElement={
                                <>
                                            ...
                                            <div>
                                                <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1); }}>
                                                    ...
                                                    <SelectContent>
                                                        ...
                                                        <SelectItem value="Waiting for Admin Approval">In Review</SelectItem>
                                ...
                                {items.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={5} className="h-32 text-center text-gray-500">
                                            No student request history found.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    items.map((item) => (
                                                ...
                                                <TableRow className="cursor-pointer" onClick={() => openViewModal(item)}>
                                                    ...
                                                    <TableCell className="text-center">
                                                        <div className="inline-grid place-items-center">
                                                            <span className={`col-start-1 row-start-1 font-semibold whitespace-nowrap ${getStatusColor(item.toStatus)}`}>
