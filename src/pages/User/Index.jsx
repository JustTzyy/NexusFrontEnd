...
export default function UsersIndex() {
    ...
    const handlePrintPDF = () => {
        generatePDF({
            ...
            companyName: "Users Management System",
        });
    };

    /* ------------------------------ routes ------------------------------ */
    const goCreate = () => navigate("/users/create");
    const goEdit = (id) => navigate(`/users/${id}/edit`);
    const goArchive = () => navigate("/users/archive");
    ...
    /* ------------------------------ actions ------------------------------ */
    const openDeleteModal = (u) => setDeleteModal({ open: true, user: u });
    const openViewModal = (u) => setViewModal({ open: true, user: u });
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setAlert({ show: true, type: "success", message: `User "${u.name}" moved to archive successfully!` });
            loadUsers(); // Refresh the list
        } catch (err) {
            setAlert({ show: true, type: "error", message: err.message || "Failed to delete user" });
    ...
    const confirmView = () => {
        if (viewModal.user) {
            navigate(`/users/${viewModal.user.id}`);
        }
    ...
    return (
        <AppLayout title="Users" onPrint={filtered.length > 0 ? handlePrintPDF : undefined}>
            {initialLoading ? (
                <TableIndexSkeleton
                    columns={5}
                    rows={10}
                    headers={["Name", "Role", "Email", "Created At", "Actions"]}
                />
            ) : (
                        ...
                        <DateRangeFilter
                            ...
                            rightElement={
                                <>
                                    ...
                                    <PermissionGate permission="ArchiveUsers">
                                        <div className="ml-auto">
                                            <Tooltip>
                                                ...
                                                <TooltipContent>View archived users</TooltipContent>
                                            </Tooltip>
                                        </div>
                                    </PermissionGate>
                                </>
                                        ...
                                        <Tooltip key={u.id}>
                                            <TooltipTrigger asChild>
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="text-gray-600">{u.createdAt}</TableCell>
                                                    <TableCell className="text-center">
                                                        <div className="flex items-center justify-center gap-1">
                                                            <PermissionGate permission="UpdateUsers">
                                                                <Tooltip>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            variant="ghost"
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            ...
                            <span className="font-semibold">{viewModal.user?.name}</span>?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
