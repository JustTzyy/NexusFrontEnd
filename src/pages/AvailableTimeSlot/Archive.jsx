...
export default function AvailableTimeSlotsArchive() {
    ...
    const [restoreModal, setRestoreModal] = useState({ open: false, slot: null });
    ...
    const handlePrintPDF = () => {
        generatePDF({
            title: "Archived Time Slots Report",
            data: filtered,
            columns: [
                { header: "Time Slot", key: "label" },
            ...
            companyName: "Archived Time Slots Management System",
        });
    };

    const handlePrintSinglePDF = (slot) => {
        if (!slot) return;
        generatePersonalInfoPDF({
            title: "Archived Time Slot Details",
            data: slot,
            fields: [
    ...
    const loadArchived = async () => {
        ...
        setError("");
        try {
            const response = await availableTimeSlotService.getArchived({ pageNumber: 1, pageSize: 100 });
            ...
            setError(err.response?.data?.message || err.message || "Failed to load archived time slots");
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmView = () => {
        if (viewModal.slot) {
            navigate(`/available-time-slots/${viewModal.slot.id}`, { state: { from: "archive" } });
        }
        setViewModal({ open: false, slot: null });
    };

    const confirmRestore = async () => {
        const s = restoreModal.slot;
        if (!s) return;
        setRestoreModal({ open: false, slot: null });
        try {
            await availableTimeSlotService.restore(s.id);
            setSlots((prev) => prev.filter((item) => item.id !== s.id));
            setAlert({ show: true, type: "success", message: `"${s.label}" restored successfully!` });
        } catch (err) {
            setAlert({ show: true, type: "error", message: err.response?.data?.message || "Failed to restore time slot" });
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            await availableTimeSlotService.permanentDelete(s.id);
            setSlots((prev) => prev.filter((item) => item.id !== s.id));
            setAlert({ show: true, type: "success", message: `"${s.label}" permanently deleted!` });
        } catch (err) {
            setAlert({ show: true, type: "error", message: err.response?.data?.message || "Failed to delete time slot" });
        }
    };

    /* ------------------------- filtering ------------------------- */
    const filtered = useMemo(() => {
        return applyFilters(slots, { search: q, searchFields: ['label'], fromDate, toDate, dateField: 'deletedAt' });
    }, [slots, q, fromDate, toDate]);
    ...
    return (
        <AppLayout title="Archived Time Slots" onPrint={filtered.length > 0 ? handlePrintPDF : undefined}>
            {loading ? (
                <TableIndexSkeleton columns={5} rows={10} headers={["Time Slot", "Status", "Start Time", "Deleted At", "Actions"]} />
            ) : (
                <div className="space-y-6">
                    {/* Header */}
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Archived Time Slots</h1>
                            <p className="mt-1 text-sm text-gray-600">View and manage archived time slots</p>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent>Return to active time slots</TooltipContent>
                            </Tooltip>
                        </div>
                    </div>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="relative">
                                            ...
                                            <Input placeholder="Search archived time slots..." value={q} onChange={(e) => { setQ(e.target.value); resetPage(); }} className="pl-10 w-[240px]" />
                                        </div>
                                    </TooltipTrigger>
                    ...
                    <div className="border rounded-lg overflow-hidden bg-white">
                        <Table>
                            ...
                            <TableBody>
                                {paged.length === 0 ? (
                                    <TableRow><TableCell colSpan={5} className="h-32 text-center text-gray-500">No archived time slots found.</TableCell></TableRow>
                                ) : (
                                    paged.map((s) => (
                                        <Tooltip key={s.id}>
                                            <TooltipTrigger asChild>
                                                <TableRow className="cursor-pointer" onClick={() => setViewModal({ open: true, slot: s })}>
                                                    ...
                                                    <TableCell className="text-gray-600 text-center">{s.deletedAt}</TableCell>
                                                    <TableCell className="text-center">
                                                        <div className="flex items-center justify-center gap-1">
                                                            <PermissionGate permission="RestoreAvailableTimeSlots"><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setRestoreModal({ open: true, slot: s }); }} className="text-green-600 hover:text-green-700 hover:bg-green-50"><RotateCcw className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent>Restore time slot</TooltipContent></Tooltip></PermissionGate>
                                                            <PermissionGate permission="PermanentDeleteAvailableTimeSlots"><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setDeleteModal({ open: true, slot: s }); }} className="text-red-600 hover:text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent>Permanently delete</TooltipContent></Tooltip></PermissionGate>
                                                        </div>
                                                    </TableCell>
                                                </TableRow>
                                            </TooltipTrigger>
            ...
            <Dialog open={viewModal.open} onOpenChange={(open) => !open && setViewModal({ open: false, slot: null })}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>View Time Slot Details</DialogTitle>
                        <DialogDescription>
                            You are about to view the archived time slot{" "}
                            <span className="font-semibold text-gray-900">"{viewModal.slot?.label}"</span>. Do you want to continue?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="outline" onClick={() => handlePrintSinglePDF(viewModal.slot)}>
                                    <Printer className="h-4 w-4 mr-2" />
                                    Print
                                </Button>
                            </TooltipTrigger>
                            <TooltipContent>Print time slot details as PDF</TooltipContent>
                        </Tooltip>
            ...
            <AlertDialog open={restoreModal.open} onOpenChange={(open) => !open && setRestoreModal({ open: false, slot: null })}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Confirm Restore</AlertDialogTitle>
                        <AlertDialogDescription>
                            Are you sure you want to restore{" "}
                            <span className="font-semibold text-gray-900">"{restoreModal.slot?.label}"</span>? This will move it back to active time slots.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <span className="font-semibold text-gray-900">"{deleteModal.slot?.label}"</span>? This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
