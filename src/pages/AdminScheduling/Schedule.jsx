...
const getStatusStyle = (status) => {
    switch (status) {
        ...
        case "Cancelled by Student": return "bg-red-600";
        default: return "bg-gray-500";
    }
};

export default function AdminSchedulingSchedule() {
    const { id } = useParams();
    const navigate = useNavigate();
    const location = useLocation();

    // Teacher passed from View page
    ...
    const [teacherAvailableDayIds, setTeacherAvailableDayIds] = useState(new Set()); // Set<dayId>
    ...
    useEffect(() => {
        ...
        if (!teacher) {
            navigate(`/admin-scheduling/${id}`, { replace: true });
            return;
        }
        ...
        <AppLayout title="Schedule Session">
            <div className="space-y-6">
                {loading ? (
                    ...
                    <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>Request not found.</AlertDescription></Alert>
                ) : (
                    <>
                        {/* Header */}
                        <div className="flex items-center gap-3">
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button variant="outline" size="icon" onClick={() => navigate(`/admin-scheduling/${id}`)}>
                        ...
                        <div className="border rounded-lg bg-white p-6 space-y-6">
                            ...
                            <div className="flex items-center justify-end gap-3">
                                <Button variant="outline" onClick={() => navigate(`/admin-scheduling/${id}`)}>
                                    Cancel
                                </Button>
                                <Button
