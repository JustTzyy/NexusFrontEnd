...
export default function BuildingsIndex() {
    ...
    /* ------------------------------ routes ------------------------------ */
    const goCreate = () => navigate("/buildings/create");
    const goEdit = (id) => navigate(`/buildings/${id}/edit`);
    const goArchive = () => navigate("/buildings/archive");
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setAlert({ show: true, type: "success", message: `Building "${b.name}" archived successfully!` });
        } catch (err) {
            setAlert({ show: true, type: "error", message: err.response?.data?.message || "Failed to archive building" });
        }
    };

    const confirmView = () => {
        if (viewModal.building) {
            navigate(`/buildings/${viewModal.building.id}`);
        }
    ...
    return (
        <AppLayout title="Buildings" onPrint={filtered.length > 0 ? handlePrintPDF : undefined}>
            {loading ? (
                <TableIndexSkeleton
                    columns={5}
                    rows={10}
                    headers={["Name", "IsActive", "Managed By", "Created At", "Actions"]}
                />
            ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-600">
                                Manage school buildings and infrastructure
                            </p>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <PermissionGate permission="CreateBuildings">
                                <Tooltip>
                                    ...
                                    <TooltipContent>Create a new building</TooltipContent>
                                </Tooltip>
                            </PermissionGate>
                        </div>
                    </div>
                        ...
                        <DateRangeFilter
                            ...
                            rightElement={
                                <>
                                    ...
                                    <PermissionGate permission="ArchiveBuildings">
                                        <div className="ml-auto">
                                            <Tooltip>
                                                ...
                                                <TooltipContent>View archived buildings</TooltipContent>
                                            </Tooltip>
                                        </div>
                                    </PermissionGate>
                                </>
                                        ...
                                        <Tooltip key={b.id}>
                                            <TooltipTrigger asChild>
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="max-w-md truncate">{b.managedBy || "—"}</TableCell>
                                                    <TableCell className="text-gray-600 text-center">{b.createdAt}</TableCell>
                                                    <TableCell className="text-center">
                                                        <div className="flex items-center justify-center gap-1">
                                                            <PermissionGate permission="UpdateBuildings">
                                                                <Tooltip>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            variant="ghost"
                                                                    ...
                                                                    <TooltipContent>Edit building</TooltipContent>
                                                                </Tooltip>
                                                            </PermissionGate>
                                                            <PermissionGate permission="DeleteBuildings">
                                                                <Tooltip>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                            variant="ghost"
                                                                    ...
                                                                    <TooltipContent>Archive building</TooltipContent>
                                                                </Tooltip>
                                                            </PermissionGate>
                                                        </div>
                                                    </TableCell>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <span className="font-semibold">{deleteModal.building?.name}</span>?
            ...
            <Dialog open={viewModal.open} onOpenChange={(open) => !open && setViewModal({ open: false, building: null })}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>View Building</DialogTitle>
                        <DialogDescription>
                            Would you like to view the details of{" "}
                            <span className="font-semibold">{viewModal.building?.name}</span>?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
